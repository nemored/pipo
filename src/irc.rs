...
use crate::{Attachment, Bus, Message, Router, Transport, TransportId, MessageId, Database};
use anyhow::anyhow;
...
impl Display for IrcId {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		write!(f, "None")
	}
}

impl FromSql for IrcId {
...
impl ToSql for IrcId {
	fn to_sql(&self) -> rusqlite::Result<rusqlite::types::ToSqlOutput<'_>> {
		self.value.to_sql()
	}
}

impl MessageId for IrcId {
	const COLUMN: &'static str = "ircid";
}
...
impl IRC {
    ...
    pub async fn connect(&mut self) -> anyhow::Result<()> {
        loop {
            let (client, mut irc_stream) = self.connect_irc().await?;
    ...
    async fn connect_irc(&mut self) -> anyhow::Result<(Client, irc::client::ClientStream)> {
    ...
    async fn try_insert_into_messages_table(&self) -> anyhow::Result<Option<i64>> {
		let id: Option<IrcId> = None;
        Ok(match &self.database {
            Some(db) => {
                let pipo_id = db.insert_into_messages_table(id).await?;
