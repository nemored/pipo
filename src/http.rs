...
use axum::{body::Body, http::{header, HeaderValue, Request, Response, StatusCode}, routing::get, Router};
use bytes::BytesMut;
use ruma::api::{client::error::{ErrorBody, ErrorKind}, OutgoingResponse};
...
impl Http {
    ...
    pub fn add_matrix_route(&mut self, hs_token: &str) {
        self.app = self
            ...
            .route_layer(ValidateRequestHeaderLayer::custom(MatrixBearer::new(hs_token)));
    }
...
mod tests {
    ...
    async fn matrix_handle_invalid_token() {
        ...
        assert_eq!(format!("{:?}", res.body()), format!("{:?}", expected.body()));
    }
}
