...
use serenity::model::prelude::{ChannelId, MessageId as SerenityMessageId};
use tokio::{net::TcpStream, sync::mpsc, task::JoinHandle};
use tokio_stream::{wrappers::ReceiverStream, StreamExt as TokioStreamExt, StreamMap};
use tokio_tungstenite::*;

use crate::{
    discord::DiscordId, Bus, Database, Message, MessageId, Router, Transport, TransportId,
};
...
pub(crate) struct SlackId {
    timestamp: Timestamp,
}

impl AsRef<Timestamp> for SlackId {
    fn as_ref(&self) -> &Timestamp {
        &self.timestamp
    }
}

impl Display for SlackId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.timestamp.fmt(f)
    }
}

impl From<Timestamp> for SlackId {
    fn from(value: Timestamp) -> Self {
        Self { timestamp: value }
    }
}

impl FromSql for SlackId {
    fn column_result(value: rusqlite::types::ValueRef<'_>) -> rusqlite::types::FromSqlResult<Self> {
        use rusqlite::types::{FromSqlError, ValueRef::*};
        match value {
            Text(s) => {
                let bytes = s.into();
                let string = String::from_utf8(bytes).map_err(|_| FromSqlError::InvalidType)?;
                let timestamp = Timestamp(string);
                Ok(Self { timestamp })
            }
            Blob(_) | Integer(_) | Null | Real(_) => Err(FromSqlError::InvalidType),
        }
    }
}

impl ToSql for SlackId {
	fn to_sql(&self) -> rusqlite::Result<rusqlite::types::ToSqlOutput<'_>> {
		self.timestamp.0.to_sql()
	}
}

impl MessageId for SlackId {
    const COLUMN: &'static str = "slackid";
}
    ...
    fn start(mut self) -> JoinHandle<()> {
        tokio::spawn(async move {
            match self.connect().await {
                ...
                Err(e) => eprintln!("Slack::connect() exited with Error: {e:#}"),
            }
        })
    }
}
...
impl Slack {
    pub async fn new(
        ...
        let channel_map: HashMap<String, (Arc<Bus>, Router)> = channel_mapping
            .iter()
            .map(|(channelname, bus)| (channelname.as_ref().clone(), (bus.clone(), router.clone())))
            .collect();
    ...
    async fn add_reaction(
        ...
        _thread: Option<(Option<Timestamp>, Option<ChannelId>)>,
    ) -> anyhow::Result<()> {
        let mut headers = HeaderMap::new();
        ...
        let ts = match self.try_select_slackid_from_messages(pipo_id).await? {
            Some(ts) => ts,
    ...
    async fn delete_message(&mut self, pipo_id: Option<i64>, channel: &str) -> anyhow::Result<()> {
        let mut headers = HeaderMap::new();
        let channel = match self.channel_id_map.get(channel) {
            Some(s) => s,
            None => return Err(anyhow!("Could not find id for channel {}", channel)),
    ...
    async fn pins_add(&mut self, channel: &str, pipo_id: Option<i64>) -> anyhow::Result<()> {
        let mut headers = HeaderMap::new();
        let channel = match self.channel_id_map.get(channel) {
            Some(s) => s,
            None => return Err(anyhow!("Could not find id for channel {}", channel)),
    ...
    async fn pins_remove(&mut self, channel: &str, pipo_id: Option<i64>) -> anyhow::Result<()> {
        let mut headers = HeaderMap::new();
        let channel = match self.channel_id_map.get(channel) {
            Some(s) => s,
            None => return Err(anyhow!("Could not find id for channel {}", channel)),
    ...
    async fn post_action_message(
        ...
        if is_edit {
            if let Some(ts) = self.try_select_slackid_from_messages(pipo_id).await? {
                return self
                    .update(
                        ts,
                        channel,
    ...
    async fn post_names_message(
        ...
        if message.as_str() == "/names" {
            ...
            let channel = match self.id_map.get(channel_id) {
                ...
                None => {
                    return Err(anyhow!(
                        "Couldn't find channel for channel id: {}",
                        channel_id
                    ));
                }
            };
    ...
    async fn post_text_message(
        ...
        let thread_ts = match thread {
            Some((_, d)) => match d {
                Some(ref d) => self.get_timestamp_for_discordid(&SerenityMessageId(*d.as_u64())).await?,
                None => None,
            },
            None => None,
        };
    ...
    async fn post_message(
        ...
        if let Some(ts) = json["ts"].as_str() {
            let ts = Timestamp(ts.to_string());
            if let Some(pipo_id) = pipo_id {
                self.update_messages(pipo_id, &ts).await?;
            }
        }
    ...
    async fn handle_bot_message(
        ...
        let _thread = match thread_ts {
            Some(ts) => Some((
                Some(ts.clone()),
                self.try_select_discordid_from_messages(&ts).await?,
            )),
            None => None,
        };
        let pipo_id = match ts {
            Some(ts) => match self.try_select_id_from_messages(&ts.clone().into()).await {
                Some(id) => Some(id),
                None => self.try_insert_into_messages_table(&ts.into()).await?,
            },
    ...
    async fn handle_message_deleted(
        ...
        let pipo_id = match ts {
            Some(ts) => match self.try_select_id_from_messages(&ts.clone().into()).await {
                Some(id) => Some(id),
                None => self.try_insert_into_messages_table(&ts).await?,
            },
            None => return Err(anyhow!("Message has no timestamp.")),
        };
    ...
    async fn try_insert_into_messages_table(&self, ts: &Timestamp) -> anyhow::Result<Option<i64>> {
        let id: SlackId = ts.clone().into();
        match &self.database {
            Some(db) => Some(db.insert_into_messages_table(Some(id)).await),
            None => None,
        }
        .transpose()
    }

    async fn update_messages(&self, pipo_id: i64, ts: &Timestamp) -> anyhow::Result<()> {
        let id: SlackId = ts.clone().into();
        match &self.database {
            Some(db) => db.update_messages_table(pipo_id, id).await,
            None => Err(anyhow!("no database")),
        }
    }
    ...
    async fn try_select_id_from_messages(&self, ts: &Timestamp) -> Option<i64> {
        let id: SlackId = ts.clone().into();
        match &self.database {
    ...
    async fn try_select_slackid_from_messages(
        &self,
        pipo_id: Option<i64>,
    ) -> anyhow::Result<Option<SlackId>> {
        if let Some(pipo_id) = pipo_id {
            match &self.database {
    ...
    async fn get_timestamp_for_discordid(
        &self,
        id: &SerenityMessageId,
    ) -> anyhow::Result<Option<Timestamp>> {
        let id: DiscordId = id.clone().into();
        match &self.database {
            Some(db) => db.get_messageid_from_messageid(id).await,
            None => Ok(None),
        }
        .map(|x| x.map(|a: SlackId| a.timestamp))
    }
    ...
    async fn handle_message(
        ...
        let pipo_id = match ts {
            Some(ts) => match self.try_select_id_from_messages(&ts).await {
                Some(id) => Some(id),
                None => self.try_insert_into_messages_table(&ts).await?,
            },
            None => return Err(anyhow!("Message has no timestamp.")),
    ...
    async fn handle_attachments(&mut self, attachments: Vec<Attachment>) -> Vec<crate::Attachment> {
        ...
        for attachment in attachments {
            let pipo_id = match attachment.ts {
                Some(ts) => self.try_select_id_from_messages(&ts).await,
                None => None,
            };
    ...
    async fn prepare_attachments_for_slack(
        ...
        for attachment in attachments {
            let ts = match self
                .try_select_slackid_from_messages(attachment.pipo_id)
                .await
            {
                Ok(ts) => ts.map(|s| s.as_ref().clone()),
                Err(_) => None,
            };
