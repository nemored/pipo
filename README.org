* Pipo
#+BEGIN_EXAMPLE
__/\\\\\\\\\\\\\____/\\\\\\\\\\\__/\\\\\\\\\\\\\_________/\\\\\______
 _\/\\\/////////\\\_\/////\\\///__\/\\\/////////\\\_____/\\\///\\\____
  _\/\\\_______\/\\\_____\/\\\_____\/\\\_______\/\\\___/\\\/__\///\\\__
   _\/\\\\\\\\\\\\\/______\/\\\_____\/\\\\\\\\\\\\\/___/\\\______\//\\\_
    _\/\\\/////////________\/\\\_____\/\\\/////////____\/\\\_______\/\\\_
     _\/\\\_________________\/\\\_____\/\\\_____________\//\\\______/\\\__
      _\/\\\_________________\/\\\_____\/\\\______________\///\\\__/\\\____
       _\/\\\______________/\\\\\\\\\\\_\/\\\________________\///\\\\\/_____
        _\///______________\///////////__\///___________________\/////_______
#+END_EXAMPLE

Pipo is the discord bot ever made.

** Build and run
*** Rust (legacy)
#+BEGIN_SRC bash
cargo run -- path-to-config.json path-to-db.sqlite3
#+END_SRC

You can also provide the paths via environment variables:

#+BEGIN_SRC bash
CONFIG_PATH=path-to-config.json DB_PATH=path-to-db.sqlite3 cargo run
#+END_SRC

*** Go rewrite branch workflow
The Go rewrite currently focuses on CLI compatibility (argument/env parsing and usage output).

#+BEGIN_SRC bash
# Build the Go binary
go build -o pipo-go ./cmd/pipo

# Run with positional args
./pipo-go path-to-config.json path-to-db.sqlite3

# Or run with environment-variable fallback
CONFIG_PATH=path-to-config.json DB_PATH=path-to-db.sqlite3 ./pipo-go
#+END_SRC

*** Cross-language parity harness (high-risk fixtures)
Use the fixture harness to run Go and Rust fixture runners against the same
config/event corpus, then compare normalized outputs + DB side effects.

#+BEGIN_SRC bash
go run ./cmd/parity-harness -fixture docs/parity/fixtures/high-risk.json

# Integration-mode scenarios with per-scenario pass/fail + divergences
go run ./cmd/parity-harness -mode integration -fixture docs/parity/fixtures/integration.json
#+END_SRC

This currently focuses on Slack↔Discord ID mapping, edits/deletes, thread
fields, reaction routing, and integration-driver scenario comparisons.

** Tasks
*** TODO Improve translation of Slack formatting to Markdown formatting



** CI and release artifacts
- Pull requests and branch pushes run both Rust and Go validation jobs.
  - Rust: =cargo build --locked=, =cargo test --locked=, plus protobuf drift/runtime checks.
  - Go: =gofmt= verification, =go vet ./...=, =go test ./...=.
- Parity validation now runs in staged workflow lanes:
  - PR-required lane: fixture parity (=docs/parity/fixtures/high-risk.json=) + deterministic local transport integration subset (=docs/parity/fixtures/integration.json=), with no external secrets required.
  - Nightly/scheduled lane: full parity fixture + integration matrix execution with live Slack/Discord credential checks.
  - Manual dispatch lane: ad hoc full parity run for release-readiness and migration decision support.
- Promotion decisions are gated on trend metrics rather than single-run outcomes:
  - pass rate,
  - divergence frequency,
  - flaky-scenario index (rerun prevalence).
- Pushes to =main= build release binaries and upload them as workflow artifacts.
- Semver tags matching =v*= also publish a GitHub release with all Go and Rust binaries plus =SHA256SUMS=.
- Current release targets:
  - Rust: =x86_64-unknown-linux-gnu=.
  - Go: =linux/amd64=, =linux/arm64=, =darwin/amd64=, =darwin/arm64=, =windows/amd64=.

** Protobuf generation contract
- Source proto path: =protos/Mumble.proto=
- Generated output path: =src/protos=
- Regeneration command: =cargo build=
- The generated files under =src/protos= are intentionally ignored by git and are expected to be rebuilt deterministically by =build.rs= using vendored =protoc=.

** Protobuf/CI acceptance criteria
- Two clean builds in fresh environments produce identical build behavior.
- CI fails if protobuf generator drift introduces unexpected repository changes.
- CI clean-regeneration drift checks run on the checked-out commit and only fail when build steps mutate tracked files unexpectedly (not because the PR contains committed changes).
- CI uses lockfile-locked cargo invocations (e.g. =cargo build --locked=) to avoid dependency-resolution drift between environments.
- CI is expected to pass with protobuf runtime =\>= 3.7.2=.
